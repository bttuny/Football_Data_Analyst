...
class PremierLeagueCardsModel:
    ...
    def fit(self, df_cards: pd.DataFrame):
        ...
        for ref, count in ref_counts.items():
            ...
            self.referee_multipliers[ref] = float(
                weight * (raw_avg / self.league_avg_cards) + (1.0 - weight)
            )
        ...
        for team in teams:
            ...
            self.team_home_intensity[team] = (
                float(h_games["home_cards"].mean() / avg_h)
                if len(h_games) > 0
                else 1.0
            )
            self.team_away_intensity[team] = (
                float(a_games["away_cards"].mean() / avg_a)
                if len(a_games) > 0
                else 1.0
            )
    ...
    def predict_cards(
        ...
        h_factor = self.team_home_intensity.get(home_team, 1.0)
        a_factor = self.team_away_intensity.get(away_team, 1.0)
        ...
        return {
            ...
            "fair_odds_over_3_5": (
                round(1.0 / prob_over_3_5, 2) if prob_over_3_5 > 0 else 0
            ),
        }